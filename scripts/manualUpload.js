...
async function manualUpload() {
  ...
  await supabase.storage.from('course_files').upload(w1Path, w1Buffer, { upsert: true });
  const w1Url = supabase.storage.from('course_files').getPublicUrl(w1Path).data.publicUrl;
  
  if (!appState.tasks[courseId]['1'][0].files) appState.tasks[courseId]['1'][0].files = [];
  appState.tasks[courseId]['1'][0].files.push({ name: "אינפי 2 הרצאה 1.pdf", url: w1Url, path: w1Path });
  ...
  await supabase.storage.from('course_files').upload(w2Path, w2Buffer, { upsert: true });
  const w2Url = supabase.storage.from('course_files').getPublicUrl(w2Path).data.publicUrl;
  
  if (!appState.tasks[courseId]['2'][0].files) appState.tasks[courseId]['2'][0].files = [];
  appState.tasks[courseId]['2'][0].files = appState.tasks[courseId]['2'][0].files.filter(f => f.name !== "test.pdf");
  
  appState.tasks[courseId]['2'][0].files.push({ name: "אינפי 2 הרצאה 2.pdf", url: w2Url, path: w2Path });
  ...
  await supabase.storage.from('course_files').upload(w3Path, w3Buffer, { upsert: true });
  const w3Url = supabase.storage.from('course_files').getPublicUrl(w3Path).data.publicUrl;
  
  if (!appState.tasks[courseId]['3'][0].files) appState.tasks[courseId]['3'][0].files = [];
  appState.tasks[courseId]['3'][0].files.push({ name: "אינפי 2 הרצאה 3.pdf", url: w3Url, path: w3Path });

  // Update DB
  await supabase.from('user_data').update({ app_state: appState }).eq('id', userId);
  console.log("Successfully uploaded lectures 1, 2, 3 manually to app_state.");
}
manualUpload();
