...
async function wipeFiles() {
  ...
  // Wipe all files from all tasks
  for (const courseId in appState.tasks) {
    for (const week in appState.tasks[courseId]) {
      appState.tasks[courseId][week].forEach(task => {
        task.files = [];
      });
    }
  }
  ...
  console.log("Successfully wiped all corrupted files from app_state.");
}
wipeFiles();
