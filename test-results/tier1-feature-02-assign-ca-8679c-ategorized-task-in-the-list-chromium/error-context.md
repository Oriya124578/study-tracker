...
- Name: tier1-feature\02-assign-categories.spec.ts >> Assign Categories to Tasks >> should display the correct category chips on a categorized task in the list
- Location: e2e\tests\tier1-feature\02-assign-categories.spec.ts:31:3
...
Locator: getByRole('listitem').filter({ hasText: 'Categorized Task' }).getByText('Work')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('listitem').filter({ hasText: 'Categorized Task' }).getByText('Work')
