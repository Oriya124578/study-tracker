...
- Name: tier2-boundary\category-assignment.spec.ts >> Assign Categories to Tasks - Boundary & Corner Cases >> should handle unassigning the last category from a task (empty category state)
- Location: e2e\tests\tier2-boundary\category-assignment.spec.ts:37:3
...
Call log:
  - waiting for locator('text=Task with One Category')
